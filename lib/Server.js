...
class Server {
    ...
    async start(options, basePath) {
        ...
        if (so.subServer) {
            for (let name in so.subServer) {
                log.info(`[${name}]sub-server options\n${JSON.stringify(so.subServer[name], null, 2)}`);
            }
        }

        log.info(`init router`);
        let routerGetter = new RouterGetter(options, so.subServer);
        ...
        if (options.static && routerGetter.routers) {
            ...
            for (let staticPath in options.static) {
                ...
                app.use(routerGetter.middlewareSend(staticPath, options.static[staticPath]));
            }

            if (rootStaticPath && routerGetter.routers.static) {
                log.info(`static(/, ${options.static[rootStaticPath]})`);
                app.use(routerGetter.middlewareSend(rootStaticPath, options.static[rootStaticPath]));
            }
        }
        ...
        return new Promise((resolve, reject) => {
            require('http').createServer(app.callback())
                ...
                .listen(options.listen, () => {
                    ...
                    if (!options.nodeEnv) {
                        log.warn(`Notice: NODE_ENV=(blank)`);
                    }
